    ...
    <div>
      ...
      <div style={{ display:"flex", gap:4, marginBottom:16 }}>
        {shopTabs.map(tab => (
          <button key={tab.id} onClick={() => setShopTab(tab.id)} style={{
            ...
            fontFamily:"'BLKCHCRY',serif", fontSize:11,
      ...
      {shopTab === "upgrades" && (
        <div style={st.panel}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
            <div style={{ fontSize:14, fontFamily:"'BLKCHCRY',serif", color:P.textPrimary, fontWeight:700, letterSpacing:1 }}>Upgrades</div>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              <div style={{ fontSize:11, color:P.quill, fontFamily:"'Junicode',sans-serif", display:"flex", alignItems:"center", gap:3 }}><Feather size={11}/> {fmt(quills)}</div>
            ...
            {PERM_UPGRADES.map(upg => {
              ...
              return (
                <div key={upg.id} style={{
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, color:P.textPrimary, fontFamily:"'BLKCHCRY',serif", fontWeight:600 }}>{upg.name}</div>
                    <div style={{ fontSize:10, color:P.textMuted, fontFamily:"'Junicode',sans-serif" }}>{isLocked ? upg.lockedLabel : description}</div>
                  </div>
                  <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4 }}>
                    {!maxed && (
                      <div style={{ fontSize:9, color:P.textMuted, fontFamily:"'Junicode',sans-serif" }}>
                        {`${level}/${upg.maxLevel}`}
                      </div>
                    )}
                    {isLocked ? (
                      <span style={{ fontSize:10, color:P.textMuted, fontFamily:"'Junicode',sans-serif" }}>locked</span>
                    ) : maxed ? (
                      <div style={{
                      ...
                      <button onClick={() => canAfford && buyPermUpgrade(upg.id, purchaseCount, purchaseCost)} style={{
                        ...
                        fontSize:10, fontFamily:"'Junicode',sans-serif", opacity: canAfford ? 1 : 0.5,
                      }}>
      ...
      {shopTab === "design" && (
        <div>
          ...
          <div style={st.panel}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
              <div style={{ fontSize:14, fontFamily:"'BLKCHCRY',serif", color:P.textPrimary, fontWeight:700, letterSpacing:1 }}>Covers</div>
              <div style={{ fontSize:11, color:P.quill, fontFamily:"'Junicode',sans-serif", display:"flex", alignItems:"center", gap:3 }}><BookMarked size={11}/> {goldenNotebooks}</div>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {COVERS.filter(c => !c.premiumOnly || ownedCovers.includes(c.id)).map(c => {
                ...
                return (
                  <div key={c.id} style={{
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:13, color:P.textPrimary, fontFamily:"'BLKCHCRY',serif", fontWeight:600 }}>{c.name}</div>
                      <div style={{ fontSize:10, color:P.textMuted, fontFamily:"'Junicode',sans-serif" }}>×{c.mult.toFixed(2)} bonus</div>
                    </div>
                    {owned ? (
                      active ? <span style={{ fontSize:10, color:P.sage, fontFamily:"'Junicode',sans-serif" }}>✓ active</span>
                      : <button onClick={() => setActiveCoverId(c.id)} style={{ padding:"5px 12px", background:P.btnInactiveBg, border:"none", color:P.btnInactiveText, borderRadius:4, cursor:"pointer", fontSize:10, fontFamily:"'Junicode',sans-serif" }}>Equip</button>
                    ) : (
                      <button onClick={() => buyItem("cover", c)} style={{ padding:"5px 12px", background:canAfford?P.btnActiveBg:P.btnInactiveBg, color:canAfford?P.btnActiveText:P.btnInactiveText, border:"none", borderRadius:4, cursor:canAfford?"pointer":"default", fontSize:10, fontFamily:"'Junicode',sans-serif", opacity:canAfford?1:0.5 }}>
          ...
          <div style={st.panel}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
              <div style={{ fontSize:14, fontFamily:"'BLKCHCRY',serif", color:P.textPrimary, fontWeight:700, letterSpacing:1 }}>Page Styles</div>
              <div style={{ fontSize:11, color:P.quill, fontFamily:"'Junicode',sans-serif", display:"flex", alignItems:"center", gap:3 }}><BookMarked size={11}/> {goldenNotebooks}</div>
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                ...
                return (
                  <div key={p.id} style={{
                    ...
                    <div style={{ width:36, height:46, borderRadius:3, background:p.bg, border:`1.5px solid ${p.accent}60`, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center" }}>
                      <div style={{ fontSize:8, color:p.text, fontFamily:"'Junicode',sans-serif", opacity:0.6 }}>Abc</div>
                    </div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:13, color:P.textPrimary, fontFamily:"'BLKCHCRY',serif", fontWeight:600 }}>{p.name}</div>
                      <div style={{ fontSize:10, color:P.textMuted, fontFamily:"'Junicode',sans-serif" }}>×{p.mult.toFixed(2)} bonus</div>
                    </div>
                    {owned ? (
                      active ? <span style={{ fontSize:10, color:P.sage, fontFamily:"'Junicode',sans-serif" }}>✓ active</span>
                      : <button onClick={() => setActivePageId(p.id)} style={{ padding:"5px 12px", background:P.btnInactiveBg, border:"none", color:P.btnInactiveText, borderRadius:4, cursor:"pointer", fontSize:10, fontFamily:"'Junicode',sans-serif" }}>Equip</button>
                    ) : (
                      <button onClick={() => buyItem("page", p)} style={{ padding:"5px 12px", background:canAfford?P.btnActiveBg:P.btnInactiveBg, color:canAfford?P.btnActiveText:P.btnInactiveText, border:"none", borderRadius:4, cursor:canAfford?"pointer":"default", fontSize:10, fontFamily:"'Junicode',sans-serif", opacity:canAfford?1:0.5 }}>
      ...
      {shopTab === "boosts" && (
        <div style={st.panel}>
          ...
          <div style={{ fontSize:14, fontFamily:"'BLKCHCRY',serif", color:P.textPrimary, fontWeight:700, letterSpacing:1, marginBottom:12 }}>Boosts & Bundles</div>
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {IAP_PRODUCTS.map(product => {
              ...
              return (
                <div key={product.id} style={{
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:13, color:P.textPrimary, fontFamily:"'BLKCHCRY',serif", fontWeight:600 }}>{product.name}</div>
                    <div style={{ fontSize:10, color:P.textMuted, fontFamily:"'Junicode',sans-serif", lineHeight:1.4 }}>{product.desc}</div>
                  </div>
                  <div style={{ flexShrink:0 }}>
                    {alreadyOwned
                      ? <span style={{ fontSize:10, color:P.sage, fontFamily:"'Junicode',sans-serif" }}>✓ owned</span>
