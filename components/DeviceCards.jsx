...
// --- WELL MINI CARD ---
export function WellMiniCard({ well, idx, wUpg, hasManager, isEnabled, onCollect, onToggleManager, wellRef, inkMult = 1, floatData }) {
      ...
      <AnimatePresence>
        {floatData && (
          <motion.div
            ...
            animate={{ opacity: [1, 1, 0], y: -52 }}
            transition={{ duration: 1.4, ease: "easeOut", times: [0, 0.45, 1] }}
            style={{
              position: "absolute", top: 28, left: "50%", transform: "translateX(-50%)",
              pointerEvents: "none", zIndex: 20, whiteSpace: "nowrap",
              display: "flex", alignItems: "center", gap: 3,
              fontSize: 12, fontWeight: 700, fontFamily: "'Junicode',sans-serif",
      ...
      >
          ...
          <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center" }}>
            {isCollecting ? (
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:8, color:"#fff", fontFamily:"'Junicode',sans-serif", fontWeight:700, textShadow:"0 1px 3px rgba(0,0,0,0.5)" }}>...</div>
        ...
        {hasManager && (
          <button
            ...
            style={{
              padding:"2px 10px", fontSize:8, fontFamily:"'Junicode',sans-serif",
      ...
      >
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
          ...
          {press.running ? (<>
            ...
            <div style={{ fontSize:10, color:P.textMuted, fontFamily:"'Junicode',sans-serif" }}>{hasManager ? "auto" : "tap"}</div>
          )}
        </div>
      </div>
          ...
          <div style={{ position:"absolute", top:20, left:"50%", transform:"translateX(-50%)", pointerEvents:"none", zIndex:10 }}>
            <motion.div
              ...
              style={{
                ...
                display:"flex", alignItems:"center", justifyContent:"center",
                fontFamily:"'Junicode',serif", fontSize:20, fontWeight:700,
  ...
  return (
    <div style={{ background:P.surfaceBg, border:`1px solid ${P.border}`, borderRadius:8, marginBottom:8, overflow:"hidden" }}>
      <div style={{ padding:"6px 10px 4px" }}>
        <div style={{ fontSize:11, color:P.textPrimary, fontFamily:"'BLKCHCRY',serif", fontWeight:600, letterSpacing:0.5 }}>{deviceLabel}</div>
      </div>
        ...
        {upgrades.map((upgrade, i) => {
          ...
          if (isMaxed) {
            return (
              <div key={upgrade.name} style={{
                ...
                padding:"0 10px", borderBottom: rowBorder,
              }}>
                <div style={{ fontSize:9, color:P.textMuted, fontFamily:"'Junicode',sans-serif", fontWeight:700 }}>{label}</div>
                <div style={{
                  background:"#fdf5d0", border:`1px solid #dcc878`, borderRadius:4,
                  padding:"2px 4px", display:"flex", flexDirection:"column", alignItems:"center", width:72, flexShrink:0,
                }}>
                  <div style={{ fontSize:10, fontFamily:"'Junicode',sans-serif", fontWeight:700, color:P.textPrimary, lineHeight:1.2 }}>{fmtUpgradeVal(upgrade, currentVal)}</div>
          ...
          return (
            <button key={upgrade.name}
              ...
              style={{
                height: ROW_H, display:"flex", alignItems:"center", justifyContent:"space-between",
                padding:"0 10px", width:"100%", textAlign:"left",
            ...
            >
              <div style={{ fontSize:9, fontFamily:"'Junicode',sans-serif", fontWeight:700, color: canAfford ? P.textPrimary : P.textMuted }}>{label}</div>
              <div style={{
                background: canAfford ? P.btnActiveBg : P.panelBg,
                border:`1px solid ${canAfford ? P.btnActiveBg : P.border}`,
                borderRadius:4, padding:"2px 4px",
                display:"flex", flexDirection:"column", alignItems:"center", width:72, flexShrink:0,
              }}>
                <div style={{ fontSize:10, fontFamily:"'Junicode',sans-serif", fontWeight:700, color: canAfford ? P.btnActiveText : P.textPrimary, lineHeight:1.2 }}>{fmtUpgradeVal(upgrade, currentVal)}</div>
                <div style={{ fontSize:7, fontFamily:"'Junicode',sans-serif", color: canAfford ? P.btnActiveText : P.textMuted, lineHeight:1, display:"flex", alignItems:"center", gap:2 }}>
                  <Droplet size={6}/>{fmt(displayCost)}
                </div>
              </div>
            </button>
          );
